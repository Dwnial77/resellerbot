...
> 🔒 **نکته برای مخازن خصوصی (Private):** اگر مخزن شما خصوصی است، قبل از دستور `git clone` روی VPS، کلید SSH یا Personal Access Token گیت‌هاب خود را تنظیم کنید.
...
> 🔒 **امنیت:** توکن پنل‌ها در فایل دیتابیس `data/bot.db` ذخیره می‌شود؛ دسترسی به سرور و بکاپ‌های دیتابیس را محدود کنید.
...
- [ ] پنل v3.2.0 فعال است و inbound با `id=1` تعریف شده است.
    
- [ ] مقدار `XUI_API_TOKEN` در پروژه ست شده و ربات بدون خطای Auth بالا می‌آید.
    
- [ ] ادمین ریسلر X را با سقف ۱۰۰ گیگ و ریسلر Y را با ۳۰۰ گیگ روی اینباند `1` می‌سازد.
    
- [ ] ریسلر X دو سرویس ۴۰+۴۰ گیگ می‌سازد؛ درخواست سرویس سوم با حجم ۳۰ گیگ ریجکت می‌شود.
    
- [ ] ریسلر Y بدون تداخل با عملکرد X کلاینت‌های خود را می‌سازد.
    
- [ ] ریسلر X اجازه و توانایی حذف سرویس‌های ریسلر Y را ندارد.
    
- [ ] پس از ساخت کلاینت، کانفیگ VLESS و لینک سابسکریپشن با موفقیت تحویل داده می‌شود.
