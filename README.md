...
|`USAGE_ALERT_ENABLED`|ارسال اعلان خودکار هنگام مصرف ۸۰٪ / ۹۰٪ (پیش‌فرض: `false` — به‌خاطر فشار API پنل)|
|`USAGE_ALERT_INTERVAL_SECONDS`|فاصله زمانی بررسی اعلان‌ها به ثانیه (پیش‌فرض: `300`)|
...
### روش ۱ — آپدیت خودکار از GitHub (پیشنهادی)

1. در تلگرام **⬆️ آپدیت ربات** یا `/bot_update` را بزنید.
2. **📥 آپدیت از GitHub** را انتخاب کنید — ربات آخرین `resellerbot-X.Y.Z.zip` را از [Releases](https://github.com/Dwnial77/resellerbot/releases) دانلود می‌کند.
3. **اعمال و ری‌استارت** را بزنید. `.env` و `data/bot.db` حفظ می‌شوند.

تنظیمات اختیاری در `.env`:

```
GITHUB_REPO=Dwnial77/resellerbot
GITHUB_UPDATE_ENABLED=true
```

### روش ۲ — آپلود دستی ZIP

1. فایل `resellerbot-X.Y.Z.zip` را از Releases دانلود کنید.
2. **⬆️ آپدیت ربات** → **📎 آپلود ZIP** → فایل را بفرستید.
3. **اعمال و ری‌استارت**.

با `/version` نسخه فعلی را چک کنید.
...
> ⚠️ **توجه:** هر دو روش بالا از **همان updater ZIP** استفاده می‌کنند. با `git pull` دستی روی VPS قاطی نکنید.
...
6. **دستورات متنی چت:** دستوراتی مثل `/set_quota` ، `/subtract_quota` ، `/set_max_clients` ، `/set_name` و `/set_allowed_inbounds` مستقیماً از طریق چت هم با همان منطق دکمه‌ها کار می‌کنند.

7. **پشتیبان دیتابیس:** `/backup` یا دکمه **💾 پشتیبان** — ساخت بک‌آپ و دریافت فایل `.db` در تلگرام، مشاهده/دانلود بک‌آپ‌های محلی در `data/backups/`، و بازیابی از فایل `.db` یا ZIP حاوی `bot.db` (نیاز به تأیید و ری‌استارت سرویس). فایل `.env` در بک‌آپ نیست.

8. **قالب‌های سرویس:** تعریف پکیج‌های سراسری (حجم/انقضا) برای ساخت سریع سرویس روی پنل اختصاصی همان ریسلر.
...
1. **داشبورد:** دستور `/start` **سهمیه کل** (یک سقف مشترک برای همه پنل‌ها)، حجم مصرفی و باقی‌مانده را نشان می‌دهد. سقف تعداد سرویس همچنان per پنل است.
...
3. **سرویس‌های من:** دریافت لینک اتصال (VLESS + سابسکریپشن)، **دریافت QR Code اختصاصی هر کانفیگ VLESS** به صورت عکس در تلگرام، مشاهده ترافیک، **تغییر انقضا**، **افزودن ترافیک** روی سرویس موجود (از سهمیه ریسلر کم می‌شود)، **ویرایش سرویس** (ریست ترافیک با تأیید، limitIp، کامنت)، فعال/غیرفعال کردن و حذف سرویس (با تأیید مکرر).
    
4. **سیستم هوشمند اعلان مصرف (اختیاری):** با `USAGE_ALERT_ENABLED=true` وقتی مصرف سهمیه کل ریسلر یا ترافیک واقعی یک سرویس به **۸۰٪** یا **۹۰٪** برسد، ربات هشدار می‌دهد. پیش‌فرض خاموش است تا فشار API پنل کم بماند.
